# prediction function
qf_predict <- function(df, regionGBRtrue, family, final_mod, name, fileName2){
  ...
  # predict
  x <- predict(final_mod,
               quantiles = c(0.05, 0.75, 0.95),
               newdata = df2)
  x2 <- cbind(df2[,id_vars], "Lwr" = x[,1], "value" = x[,2], "Upr" = x[,3])
  dz_final <- as.data.frame(x2)
  save(dz_final, file = paste0(forecast_save, name, "_", fileName2))
}
