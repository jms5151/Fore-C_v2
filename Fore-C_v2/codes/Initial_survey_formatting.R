...
# add species names to Guam data
guam <- guamRaw %>% 
  left_join(guam.gps, by = c("Site")) %>% 
  left_join(guamCodes, by = "Species_code")

# format taxonomy
acroporids <- children("Acroporidae") # list species by coral family indicated
poritids <- children("Poritidae")

esd0812$Family <- ifelse((esd0812$Genus %in% acroporids$Acroporidae$name) == TRUE, "Acroporidae", NA)
esd0812$Family <- ifelse((esd0812$Genus %in% poritids$Poritidae$name) == TRUE, "Poritidae", esd0812$Family)

esd1317$Family <- ifelse((esd1317$Genus %in% acroporids$Acroporidae$name) == TRUE, "Acroporidae", NA)
esd1317$Family <- ifelse((esd1317$Genus %in% poritids$Poritidae$name) == TRUE, "Poritidae", esd1317$Family)

hicordis$Family <- ifelse((hicordis$Genus %in% acroporids$Acroporidae$name) == TRUE, "Acroporidae", NA)
hicordis$Family <- ifelse((hicordis$Genus %in% poritids$Poritidae$name) == TRUE, "Poritidae", hicordis$Family)

guam$Genus <- gsub("([A-Za-z]+).*", "\\1", guam$Species)
guam$Family <- ifelse((guam$Genus %in% acroporids$Acroporidae$name) == TRUE, "Acroporidae", NA)
guam$Family <- ifelse((guam$Genus %in% poritids$Poritidae$name) == TRUE, "Poritidae", guam$Family)
