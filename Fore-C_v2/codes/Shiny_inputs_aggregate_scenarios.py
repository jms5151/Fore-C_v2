...
x = agg_to_manage_zones_scenarios(scenario = ga_gbr_scenarios, management_df = management_area_poly_pix_ids)
x.to_csv(scenarios_file_dir + 'ga_gbr_scenarios_aggregated_to_management_zones.csv', index = False)

# GA Pacific management zones
x = agg_to_manage_zones_scenarios(scenario = ga_pac_scenarios, management_df = management_area_poly_pix_ids)
x.to_csv(scenarios_file_dir + 'ga_pac_scenarios_aggregated_to_management_zones.csv', index = False)

# WS GBR management zones
x = agg_to_manage_zones_scenarios(scenario = ws_gbr_scenarios, management_df = management_area_poly_pix_ids)
x.to_csv(scenarios_file_dir + 'ws_gbr_scenarios_aggregated_to_management_zones.csv', index = False)

# WS Pacific management zones
x = agg_to_manage_zones_scenarios(scenario = ws_pac_scenarios, management_df = management_area_poly_pix_ids)
x.to_csv(scenarios_file_dir + 'ws_pac_scenarios_aggregated_to_management_zones.csv', index = False)

# GA GBR GBRMPA zones
x = agg_to_manage_zones_scenarios(scenario = ga_gbr_scenarios, management_df = gbrmpa_park_zones_poly_pix_ids)
x.to_csv(scenarios_file_dir + 'ga_gbr_scenarios_aggregated_to_gbrmpa_park_zones.csv', index = False)

# WS GBR GBRMPA zones
x = agg_to_manage_zones_scenarios(scenario = ws_gbr_scenarios, management_df = gbrmpa_park_zones_poly_pix_ids)
